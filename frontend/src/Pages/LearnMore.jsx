...
function LearnMore({ isDark }) {
  ...
  const techStack = {
    ...
    Backend: [
      ...
      { name: "Express", icon: <Server className="w-4 h-4" />, color: isDark ? "text-[var(--text-muted)]" : "text-gray-700" },
  ...
  return (
    <div className="min-h-screen relative pt-36 pb-12 px-4 sm:px-6 lg:px-8 bg-transparent">
      <motion.div
      ...
      >
        ...
        <motion.div variants={itemVariants} className="text-center space-y-6">
          <motion.div
            className="inline-flex items-center justify-center p-4 rounded-3xl bg-theme-card border border-theme-border shadow-theme mb-6 text-theme-text hover:shadow-[0_0_20px_var(--color-primary)] transition-all duration-300"
            whileHover={{ scale: 1.1, rotate: 10 }}
            transition={{ type: "spring", stiffness: 300 }}
          >
            <Sparkles className="w-12 h-12 text-theme-primary" />
          </motion.div>
          <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight text-theme-text drop-shadow-xl">
            More Than Just <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-secondary)]">
              A Diary
            </span>
          </h1>
          <p className="text-lg md:text-xl max-w-3xl mx-auto leading-relaxed text-theme-text/80">
        ...
        <motion.div variants={itemVariants} className="relative">
          <div className="flex items-center justify-center gap-3 mb-20 relative z-10">
            <div className="h-px w-12 bg-theme-border"></div>
            <h2 className="text-2xl font-bold uppercase tracking-widest text-theme-text opacity-80">Journey Through Features</h2>
            <div className="h-px w-12 bg-theme-border"></div>
          </div>

          <div className="relative max-w-5xl mx-auto">
            <div className="absolute left-4 md:left-1/2 top-0 bottom-0 w-0.5 -translate-x-1/2 bg-gradient-to-b from-transparent via-[var(--color-primary)] to-transparent opacity-20" />
            ...
            {features.map((feature, index) => (
              ...
              >
                ...
                <div className="absolute left-4 md:left-1/2 -translate-x-1/2 flex items-center justify-center w-8 h-8 md:w-12 md:h-12 rounded-full border border-theme-border backdrop-blur-md z-10 shadow-lg bg-theme-card">
                  <div className={`w-3 h-3 md:w-4 md:h-4 rounded-full ${feature.bg} bg-opacity-80`} />
                </div>

                <div className="pl-16 md:pl-0 w-full md:w-5/12">
                  <ScrollCard className="p-6 md:p-8 rounded-3xl group h-full">
        ...
        <motion.div variants={itemVariants} className="relative rounded-[2.5rem] p-10 overflow-hidden border border-theme-border shadow-theme bg-theme-card/50 backdrop-blur-sm">
          <div className="relative z-10">
            <h2 className="text-3xl font-bold text-center mb-16 text-theme-text">Built With The Best</h2>

            <div className="grid md:grid-cols-2 gap-16">
              <div className="space-y-8">
                <h3 className="text-xl font-bold uppercase tracking-wider text-center text-theme-text border-b border-theme-border pb-4">
                  Frontend Ecosystem
                </h3>
                <div className="grid grid-cols-2 gap-4">
                  {techStack.Frontend.map((tech, idx) => (
                    <motion.div
                      ...
                      className="flex items-center gap-4 p-4 rounded-xl bg-theme-bg border border-theme-border hover:shadow-theme transition-all text-theme-text"
                    >
                      <div className={`p-2 rounded-lg ${tech.color} bg-opacity-10`}>
                        {tech.icon}
                      </div>
